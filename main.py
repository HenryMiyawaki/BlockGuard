...
def main():
    ...
    preprocessor = Preprocessor(file_path="datasets/Merged_dataset/w_all.csv")
    ...
    synthetic_df = preprocessor.generate_synthetic_dataset(noise_type="gaussian", noise_level=0.05)
    print("Synthetic dataset shape:", synthetic_df.shape)
    print(synthetic_df.head())

    
    # X_train, X_test, y_train, y_test = preprocessor.process()
    # synth_dataset = preprocessor.generate_synthetic_dataset()
    # print("Synthetic dataset shape:", synth_dataset.shape)
    # print(synth_dataset.head())
    
    # synth_dataset.iloc[:, :-1].plot(kind='hist', subplots=True, figsize=(12, 8))
    # plt.show()
